...
public:
    ...
    ApplyResult setNightLightDisabled(bool value) override { if(systemDelayMs>0)QThread::msleep(static_cast<unsigned long>(systemDelayMs));++nightDisableCalls;if(!nightLightSetResult.success)return nightLightSetResult;if(!nightOwned){originalNightDisabled=nightDisabled;nightOwned=true;} nightDisabled=value;operationLog<<(value?QStringLiteral("night.disable"):QStringLiteral("night.enable")); return ApplyResult::ok(); }
    ...
    ApplyResult openGpuControlPanel(const DisplayInfo &display) override { ++gpuPanelOpenCalls;lastGpuPanelDisplayId=display.stableId;return gpuPanelOpenResult; }
