...
void CoreTests::liveDisplayGpuMappingProbe(){
    ...
    for(const DisplayInfo &display:gateDisplays){const QString targetKey=QStringLiteral("%1:%2:%3").arg(display.adapterHigh).arg(display.adapterLow).arg(display.targetId);QVERIFY(!targetKeys.contains(targetKey));targetKeys.insert(targetKey);if(display.cloneMode){QVERIFY(display.cloneTargetIds.size()>1);QVERIFY(!display.clonePeerNames.isEmpty());}qInfo().noquote()<<QStringLiteral("Color gate %1: source=%2 target=%3 group=%4 targets=%5 WDDM=%6 MatrixDDI=%7 color=%8 ACM=%9/%10 Win10Candidate=%11 clone=%12 fingerprint=%13").arg(display.friendlyName).arg(display.sourceId).arg(display.targetId).arg(display.cloneGroupKey).arg(display.cloneTargetIds.size()).arg(display.wddmVersion).arg(display.matrixDdiSupported).arg(display.colorAdjustmentSupported).arg(display.acmSupported).arg(display.acmEnabled).arg(display.usesWindows10Mhc2).arg(display.cloneMode).arg(display.colorCapabilityFingerprint);foundWddm|=display.wddmVersion>0;if(display.usesWindows10Mhc2)QVERIFY(!display.colorCapabilityFingerprint.isEmpty());}
