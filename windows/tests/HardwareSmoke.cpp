...
int main(int argc,char **argv){QCoreApplication app(argc,argv);WindowsPlatformServices platform;QTextStream out(stdout),err(stderr);
    ...
    out<<platform.platformDiagnostic()<<Qt::endl;const auto displays=platform.displays();if(displays.isEmpty()){err<<"No active displays were enumerated."<<Qt::endl;return 2;}int failures=0;for(const DisplayInfo &d:displays){out<<d.friendlyName<<" | "<<d.deviceName<<" | source="<<d.sourceId<<" target="<<d.targetId<<" | Color path="<<d.colorAdjustmentSupported<<" Windows 10 MHC2="<<d.usesWindows10Mhc2<<" | ACM supported="<<d.acmSupported<<" enabled="<<d.acmEnabled<<" | profile="<<platform.defaultProfileForDiagnostics(d)<<Qt::endl;const ApplyResult apply=platform.applyToneCurve(d,ToneCurve::identity());if(!apply.success){err<<apply.error<<Qt::endl;++failures;}else{const ApplyResult restore=platform.restoreToneCurve(d);if(!restore.success){err<<restore.error<<Qt::endl;++failures;}}}
