...
int main(int argc,char **argv) {
...
#endif
    bool quitting=false;std::future<void> shutdownFuture;QTimer shutdownPoll;shutdownPoll.setInterval(16);
    QObject::connect(&shutdownPoll,&QTimer::timeout,&app,[&]{
        if(!shutdownFuture.valid()||shutdownFuture.wait_for(std::chrono::milliseconds(0))!=std::future_status::ready)return;
        try{shutdownFuture.get();}catch(...){ }
        shutdownPoll.stop();app.quit();
    });
    auto requestQuit=[&]{if(quitting)return;quitting=true;
#ifdef Q_OS_WIN
        displayEvents.stop();nightLightMaintenance.stop();trayThemeTimer.stop();
#endif
        menu.hide();welcome.hide();tray.hide();panel.beginShutdown();
        shutdownFuture=std::async(std::launch::async,[&controller]{controller.shutdown();});shutdownPoll.start();};
    ...
    auto showWelcomeOnce=[&]{
        if(quitting||!shouldShowWelcome||welcomeRequested)return;
        welcomeRequested=true;welcome.show();controller.setTrayDiscoveryShown(true,currentExecutable);
    };
    if(!backgroundLaunch){if(shouldShowWelcome)QTimer::singleShot(250,&tray,[&]{showWelcomeOnce();});else panel.showPanel();}
    const int exitIndex=arguments.indexOf(QStringLiteral("--exit-after-ms"));
    if(exitIndex>=0 && exitIndex+1<arguments.size())QTimer::singleShot(arguments[exitIndex+1].toInt(),&app,requestQuit);
