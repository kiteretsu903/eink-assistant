...
and known limitations. It is intentionally more detailed than `WINDOWS.md`.
...
- honest about unavailable features: unsupported controls are absent instead
  of being present but misleadingly disabled;
...
### 2.2 Use public APIs unless a private integration is isolated and optional

Display gamma, ICC/WCS, Task Scheduler, registry theme values, display
enumeration, and accessibility settings use Windows APIs. The notification-area
promotion/anchor path is the deliberate exception: Windows 11 provides no
public API for promoting a tray icon into the visible area, so a narrow private
Explorer integration is used. It must always fail safely and fall back to a
normal tray icon and approximate callout placement.
...
### 2.6 Unsupported features are hidden, not disabled

Capability checks happen both in the UI and the Windows backend. A feature that
is unavailable must not be constructed as a visible control, and the backend
must still reject an accidental call. This two-layer rule prevents a UI error
from reaching an unsupported API.

Do not replace absence with a permanently disabled row unless the product
...
Current Windows 2.3 presets are:
...
These values were synchronized from the re-released upstream v2.3 presets.
...
license are included in `THIRD-PARTY-NOTICES-WINDOWS.md`.
...
callout use a translucent background and antialiased rounded-body painting. The
welcome callout paints the arrow and body as one visual surface.
...
The welcome callout follows the same bind-before-show rule and delayed
reposition. Never position using physical pixels from one monitor and Qt
logical coordinates from another.
...
Startup may use `BusyDialog` before the panel is constructed. Quit shows the
inline restoring state immediately, runs controller restoration on a worker,
and pumps non-user-input events so the final frame remains responsive.
...
- A `QLockFile` named `EinkAssistant.lock` in the temporary directory enforces
  one instance, with stale-lock recovery.
...
- Quit restores temporary display state before removing the tray icon.
...
taskbars, maps the matching icon to the correct monitor/DPI space, and falls
back to `QSystemTrayIcon::geometry()` when private inspection is unavailable.

The private Explorer structure can change in a Windows update. Failure must
never prevent application startup, hide the only way to reopen the panel, or
write unrelated notification settings.

Moving or rebuilding the executable changes its exact path and can legitimately
trigger discovery again. An enabled Launch-at-Login task is automatically
reconciled to the current real application path on the next successful launch.
...
Qt/MinGW DLLs and plugins, licenses, `WINDOWS.md`, and this engineering reference
...
1. Read this document, `WINDOWS.md`, `PlatformServices.h`, the relevant Windows
...
- [ ] Verify unsupported controls are absent on simulated/real older Windows.
...
- [ ] Verify first-run callout arrow points to the actual tray icon on the
      correct taskbar.
...
- [ ] Package contains executable, minimal DLL/plugin set, licenses,
      `WINDOWS.md`, and `WINDOWS-TECHNICAL.md`.
