...
主要なディスプレイ制御に権限は不要です。アプリ以外のバックグラウンドサービスもありません。
MITライセンスです。
...
- Macのロック中にディスプレイ自動化または終了時のクリーンアップが動くと表示されることがあった
  「ショートカット」の警告を修正しました。
- 明確に異なる名前でロック中のバックグラウンド実行に対応した新しい補助ショートカットを採用し、
  アップグレード時に互換性のないv2.0版を誤って再利用しないようにしました。
- 既存ユーザーはアップグレード後に**ショートカットを追加**を1回確認してください。その後、古い
  **E-Ink Assistant Accessibility Helper**は「ショートカット」から削除できます。
...
- **ディスプレイごとの制御**：登録した電子ペーパーパネルだけを調整し、他の画面には触れません。
...
- **透明度と視差効果を下げる**：システム表示を簡素化し、低速更新画面が苦手な動きを減らします。
...
システム全体の**透明度と視差効果を下げる**機能には、初回だけ**インストールしてオン**の設定が必要です。
