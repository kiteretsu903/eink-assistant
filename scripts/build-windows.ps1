...
$outputDir = Join-Path $repoRoot 'artifacts\E-Ink-Assistant-Windows'
...
& $cmake -S $repoRoot -B $buildDir -G Ninja "-DCMAKE_BUILD_TYPE=$Configuration" "-DCMAKE_PREFIX_PATH=$qtPrefix"
...
Copy-Item -LiteralPath (Join-Path $repoRoot 'WINDOWS.md') -Destination $outputDir -Force
Copy-Item -LiteralPath (Join-Path $repoRoot 'WINDOWS-TECHNICAL.md') -Destination $outputDir -Force
Copy-Item -LiteralPath (Join-Path $repoRoot 'THIRD-PARTY-NOTICES-WINDOWS.md') -Destination $outputDir -Force
Copy-Item -LiteralPath (Join-Path $repoRoot 'licenses\LGPL-3.0.txt') -Destination $outputDir -Force
...
$zipPath = Join-Path $repoRoot 'artifacts\E-Ink-Assistant-Windows.zip'
