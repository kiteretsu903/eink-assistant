...
- 扩展并重新定位为同时服务黑白和彩色墨水屏。
- 新增逐显示器 RGB 控制，每个通道可在 0%–200% 范围内调节。
- 新增安全的辅助功能开／关捷径，可选择自动跟随显示器连接状态，并在应用退出时自动关闭。
- 使用更大文字、硬边框、更醒目的控件和更清晰的选中状态重新设计界面，适合反射式屏幕。
...
并以“无”操作结束，因此不会产生输出。它不会出现在共享表单、Spotlight、快速操作或锁定状态中；
应用也不会列出或查看你的其他快捷指令。
