...
**只对这一台显示器生效。** 你的笔记本内屏和其他显示器仍然保留 Night Shift 与原彩显示：这是针对
单个显示器的覆盖设置，不是系统设置里的全局开关。

需要管理员密码，并且**重启后**才生效。这是本应用中唯一**不会**在退出时还原的设置，因为还原它同样
需要再输一次密码并再重启一次。原有的显示器覆盖设置（例如自定义缩放分辨率）会被保留。
...
预设 **原厂 / 增强 / 艳丽 / 动漫**，也可以用滑块在 60%–300% 之间调节。
...
所有数值都是在一台 **Bigme B251 Pro**（固件版本 R2 FW V2.0）上调校的，配置为：
**网页模式、硬件伽马“等级3”、对比度 50、色彩还原模式关闭**。

原理本身并不局限于这块屏幕，任何 macOS 能指定颜色配置文件的显示器都适用，但预设值是对着这一块
屏幕用眼睛调出来的。其他墨水屏显示器可能同样受益，**请自行斟酌使用，风险自负**。
