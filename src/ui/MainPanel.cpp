...
MainPanel::MainPanel(ApplicationController *controller,QWidget *parent):QWidget(parent),m_controller(controller) {
    ...
    auto *minimize=ui::outlinedButton(QStringLiteral("−"),QStringLiteral("minimize-button"));minimize->setFixedWidth(42);minimize->setToolTip(L("minimize"));minimize->setAccessibleName(L("minimize"));connect(minimize,&QPushButton::clicked,this,&QWidget::hide);headerLayout->addWidget(minimize);auto *quit=ui::outlinedButton(L("quit"),QStringLiteral("quit-button"));connect(quit,&QPushButton::clicked,this,&MainPanel::quitRequested);headerLayout->addWidget(quit);root->addWidget(header);root->addWidget(ui::divider());
...
QWidget *MainPanel::windowsLightModeSection() {
    ...
    v->addWidget(makeLabel(m_controller->platform().windowsLightModeAvailable()?L("system.lightMode.note"):L("system.lightMode.unavailable"),false,true));
    return w;
}

QWidget *MainPanel::nightLightControlSection() {
    auto *w=new QWidget;auto *v=new QVBoxLayout(w);v->setContentsMargins(0,0,0,0);v->setSpacing(7);
    const bool initiallyDisabled=!m_controller->platform().nightLightEnabled();auto *row=new QHBoxLayout;row->addWidget(makeLabel(L("night.disable.title"),true));auto *status=makeLabel(initiallyDisabled?L("system.on"):L("system.off"),false,true);status->setObjectName(QStringLiteral("disable-night-light-status"));row->addWidget(status);row->addStretch();auto *toggle=new EinkSwitch;toggle->setObjectName(QStringLiteral("disable-night-light-switch"));toggle->setChecked(initiallyDisabled);connect(toggle,&QAbstractButton::toggled,this,[this](bool on){m_controller->setNightLightDisabled(on);});row->addWidget(toggle);v->addLayout(row);
    v->addWidget(makeLabel(L("night.disable.note"),false,true));auto *warning=makeLabel(QStringLiteral("⚠ ")+L("night.disable.warning"));warning->setObjectName(QStringLiteral("night-light-warning"));warning->setProperty("preserve-hidden",true);warning->setStyleSheet(QStringLiteral("color:#a64b00;font-weight:700;"));warning->setVisible(!initiallyDisabled);v->addWidget(warning);
    connect(m_controller,&ApplicationController::nightLightStateChanged,toggle,[toggle,status,warning](bool disabled){QSignalBlocker blocker(toggle);toggle->setChecked(disabled);status->setText(disabled?L("system.on"):L("system.off"));warning->setVisible(!disabled);});return w;
}
