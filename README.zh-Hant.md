...
電子紙助手是一款小巧的選單列 App，可逐台顯示器改善這些問題，不影響 MacBook 內建螢幕。彩色電子紙
還能使用彩度與直接 RGB 調校。

核心顯示控制不需要權限。除了 App 本身之外沒有背景服務。採用 MIT 授權。

## 最新版本

**v2.0 — 2026 年 8 月 13 日**

- 擴充並重新定位為同時支援黑白與彩色電子紙。
- 新增逐顯示器 RGB 控制，每個色彩通道可在 0%–200% 範圍內調整。
- 新增安全的輔助使用開啟／關閉捷徑，可選擇自動跟隨顯示器連線狀態，並在 App 結束時自動關閉。
- 以更大文字、硬邊框、更醒目的控制項和更清楚的選取狀態重新設計介面，適合反射式螢幕。
...
- **安全的生命週期**：結束時還原顯示調整，啟動時重新套用；輔助使用設定也會在結束時關閉。
...
並以「無」動作結束，因此不會產生輸出。它不會出現在分享表單、Spotlight、快速動作或鎖定狀態中；
App 也不會列出或檢查你的其他捷徑。
...
自己的數值；進階模式為此提供完整曲線。自訂調校請自行承擔風險。
