...
- 白黒・カラー両方の電子ペーパーディスプレイ向けに対応範囲と説明を拡張しました。
- ディスプレイごとに各チャンネル0%〜200%のRGB直接調整を追加しました。
- 安全なアクセシビリティのオン／オフ用ショートカット、任意のディスプレイ接続連動、終了時の
  自動オフを追加しました。
- 反射型画面向けに、大きな文字、明確な枠線、見やすい操作部、明瞭な選択状態で画面を再設計しました。
...
`Nothing`で終えるため出力はありません。共有シート、Spotlight、クイックアクション、ロック中の実行には
表示されず、アプリが他のショートカットを一覧表示または確認することもありません。
